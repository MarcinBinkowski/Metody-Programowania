...
(define (clause-set-add rc rc-set)
  (define (eq-cl? sc) ; sprawdzamy tutaj czy pozytywne i negatynwe literaly w naszej klauzuli sa takie same (Chyba wedlug poradku alfabetycznego sa posortowane)
...
(define (eq-lit? lit lit2)
  (and (lit? lit)
       (lit? lit2)
       (eq? (lit-var lit) (lit-var lit2))
       (eq? (lit-pol lit) (lit-pol lit2))))

(define (subsumes? c1 c2)
  (define (search x ls)
  (cond
    [(null? ls) false]
    [(eq-lit? x (car ls)) true]
    [else (search x (cdr ls))]))
  (define (subsumes-iter ls1 ls2)
    (cond
      [(null? ls1) true]
      [(search (car ls1) ls2) (subsumes-iter (cdr ls1) ls2)]
      [else false]))
  (and (clause? c1)
       (clause? c2)
       (subsumes-iter (clause-lits c1) (clause-lits c2))))
